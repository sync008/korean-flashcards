...
function generateTellingTimeCards() {
    ...
    for (let hour24 = 0; hour24 <= 23; hour24++) {
        for (let m = 0; m < minuteOptions.length; m++) {
            ...
            const hourConverted = numberToSinoKoreanHour(hour12);
            const minuteConverted = numberToSinoKoreanMinute(minute);

            const displayTime = periodKorean + ' ' + hour12 + ':' + minute.toString().padStart(2, '0');
            const spokenKorean = periodKorean + ' ' + hourConverted.korean + ' ' + minuteConverted.korean;
            const romanization = periodRoman + ' ' + hourConverted.roman + ' ' + minuteConverted.roman;
...
function loadCategory(categoryName) {
    ...
    } else if (categoryName === "Telling Time (24hr)") {
